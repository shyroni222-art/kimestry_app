...
function App() {
  ...
  const chartData = benchmarkData.map(item => ({
    ...
    columnAccuracy: item.column_accuracy ? parseFloat((item.column_accuracy * 100).toFixed(2)) : 0,
  }));
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6} md={3}>
              <Card sx={{ 
                ...
                <CardContent>
                  ...
                  <Typography variant="h3" component="div">{benchmarkData.length}</Typography>
                ...
                <CardContent>
                  ...
                  <Typography variant="h3" component="div">
                    {benchmarkData.length > 0 
                      ? (benchmarkData.reduce((sum, item) => sum + (item.accuracy || 0), 0) / benchmarkData.length * 100).toFixed(2) + '%' 
                      : '0%'}
                  </Typography>
                  <TrendingUpIcon sx={{ fontSize: 40, mt: 1 }} />
                </CardContent>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card sx={{ 
                ...
                <CardContent>
                  ...
                  <Typography variant="h3" component="div">
                    {benchmarkData.length > 0 
                      ? (benchmarkData.reduce((sum, item) => sum + (item.schema_accuracy || 0), 0) / benchmarkData.length * 100).toFixed(2) + '%' 
                      : '0%'}
                  </Typography>
                  <TrendingUpIcon sx={{ fontSize: 40, mt: 1 }} />
                </CardContent>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card sx={{ 
                ...
                <CardContent>
                  <Typography variant="h6" gutterBottom>Avg. Column Accuracy</Typography>
                  <Typography variant="h3" component="div">
                    {benchmarkData.length > 0 
                      ? (benchmarkData.reduce((sum, item) => sum + (item.column_accuracy || 0), 0) / benchmarkData.length * 100).toFixed(2) + '%' 
                      : '0%'}
                  </Typography>
                  <TrendingUpIcon sx={{ fontSize: 40, mt: 1 }} />
                </CardContent>
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="90%">
                <BarChart data={chartData}>
                  ...
                  />
                  <Legend />
                  <Bar dataKey="accuracy" name="Overall Accuracy">
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="90%">
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="columnAccuracy" name="Column Accuracy" fill="#82ca9d" />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80%' }}>
              ...
              <Table sx={{ minWidth: 650 }} aria-label="benchmark leaderboard">
                <TableHead>
                  <TableRow sx={{ backgroundColor: 'rgba(0, 0, 0, 0.2)' }}>
                    ...
                    <TableCell align="center" sx={{ color: '#fff', fontWeight: 'bold' }}>Overall Accuracy</TableCell>
                    <TableCell align="center" sx={{ color: '#fff', fontWeight: 'bold' }}>Schema Accuracy</TableCell>
                    <TableCell align="center" sx={{ color: '#fff', fontWeight: 'bold' }}>Column Accuracy</TableCell>
                    <TableCell align="center" sx={{ color: '#fff', fontWeight: 'bold' }}>Environment Accuracy</TableCell>
                  ...
                  {loading ? (
                    ...
                    benchmarkData.map((row, index) => (
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <Chip 
                            label={`${row.column_accuracy ? (row.column_accuracy * 100).toFixed(2) : 0}%`} 
                            size="small"
                            sx={{ 
                              backgroundColor: row.column_accuracy > 0.8 ? '#81C784' : 
                                               row.column_accuracy > 0.5 ? '#FFB74D' : '#E57373',
                              color: '#fff',
                              fontWeight: 'bold'
                            }}
                          />
                        ...
                        <TableCell align="center" sx={{ color: '#fff' }}>
                          {row.env_accuracy ? (row.env_accuracy * 100).toFixed(2) : 0}%
                        </TableCell>
                        <TableCell align="center" sx={{ color: '#fff' }}>
                          {row.total_tests || 0}
                        </TableCell>
